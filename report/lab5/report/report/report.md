...
![Топология простейшей сети в GNS3](image/2.png){#fig:002 width=90%}

3. Задайте IP-адреса VPCS. Для этого с помощью меню, вызываемого правой кнопкой мыши, запустите Start , например, PC-1, затем вызовите его терминал Console . Для просмотра синтаксиса возможных для ввода команд наберите /? (рис. [-@fig:003])

![Просматриваем синтаксис](image/3.png){#fig:003 width=90%}

Для задания IP-адреса 192.168.1.11 в сети 192.168.1.0/24 введите (рис. [-@fig:004])

![Задаем IP-адреса 192.168.1.11](image/4.png){#fig:004 width=90%}

Здесь 192.168.1.1 — адрес шлюза. Для уточнения синтаксиса перед вводом можно ввести ip /?. Для сохранения конфигурации необходимо ввести команду save.(рис. [-@fig:005])

![Сохраняем ](image/5.png){#fig:005 width=90%}

Аналогичным образом задайте IP-адрес 192.168.1.12 для PC-2.(рис. [-@fig:006])

![Задаем IP-адреса для PC-2](image/6.png){#fig:006 width=90%}

4. Проверьте работоспособность соединения между PC-1 и PC-2 с помощью команды ping.(рис. [-@fig:007])

![Проверяем работоспособность](image/7.png){#fig:007 width=90%}
...
1. Запустите на соединении между PC-1 и коммутатором анализатор трафика. Для этого щёлкните правой кнопкой мыши на соединении, выберете в меню Start capture , при необходимости можете скорректировать название DUMP-файла. Запустится Wireshark, а в проекте GNS3 на соединении появится значок лупы.(рис. [-@fig:008])

![Топология простейшей сети в GNS3 + Wireshark](image/9.png){#fig:008 width=90%}

2. В проекте GNS3 стартуйте все узлы (меню GNS3 Control Start/Resume all nodes ). В окне Wireshark  отобразится информация по протоколу ARP. Проанализируйте полученную информацию, дайте пояснения в отчёте.(рис. [-@fig:009])

![Информация по протоколу ARP](image/11.png){#fig:009 width=90%}
...
3. В терминале PC-2 посмотрите информацию по опциям команды ping, введя ping /?. Затем сделайте один эхо-запрос в ICMP-моде к узлу PC-1. В окне Wireshark, проанализируйте полученную информацию, дайте пояснения в отчёте.(рис. [-@fig:010])

![Эхо-запрос в ICMP-моде](image/12.png){#fig:010 width=90%}
...
4. Сделайте один эхо-запрос в UDP-моде к узлу PC-1. В окне Wireshark? проанализируйте полученную информацию, дайте пояснения в отчёте.(рис. [-@fig:013])

![Эхо-запрос в UDP-моде](image/13.png){#fig:013 width=90%}
...
5. Сделайте один эхо-запрос в TCP-моде к узлу PC-1. В окне Wireshark, проанализируйте полученную информацию, дайте пояснения в отчёте.(рис. [-@fig:014])

![Эхо-запрос в TCP-моде](image/15.png){#fig:014 width=90%}
...
2. В рабочей области GNS3 разместите VPCS, коммутатор Ethernet и маршрутизатор FRR (рис. [-@fig:015])

![Топология простейшей сети с маршрутизатором](image/16.png){#fig:015 width=90%}
...
6. Настройте IP-адресацию для интерфейса узла PC1:(рис. [-@fig:016])

![Настроиваем IP-адресацию](image/17.png){#fig:016 width=90%}

7. Настройте IP-адресацию для интерфейса локальной сети маршрутизатора:(рис. [-@fig:017]),(рис. [-@fig:018])

![Настроиваем IP-адресацию локальной сети маршрутизатора](image/18.png){#fig:017 width=90%}

![Настроиваем IP-адресацию локальной сети маршрутизатора](image/19.png){#fig:018 width=90%}

8. Проверьте конфигурацию маршрутизатора и настройки IP-адресации:(рис. [-@fig:019]),(рис. [-@fig:020])

![Проверяем конфигурацию маршрутизатора](image/20.png){#fig:019 width=90%}

![Проверяем настройки IP-адресации](image/21.png){#fig:020 width=90%}

9. Проверьте подключение. Узел PC1 должен успешно отправлять эхо-запросы на адрес маршрутизатора 192.168.1.1.(рис. [-@fig:021])

![Проверяем подключение, используем ping](image/22.png){#fig:021 width=90%}

10. В окне Wireshark проанализируйте полученную информацию, дайте пояснения в отчёте.(рис. [-@fig:022])

![Окно Wireshark](image/23.png){#fig:022 width=90%}
...
1. Запустите GNS3 VM и GNS3. Создайте новый проект.
2. В рабочей области GNS3 разместите VPCS, коммутатор Ethernet и маршрутизатор VyOS (рис. [-@fig:023])

![Топология простейшей сети с маршрутизатором](image/24.png){#fig:023 width=90%}

3. Измените отображаемые названия устройств. Коммутатору присвойте название по принципу msk-user-sw-0x, маршрутизатору — по принципу msk-user-gw-0x, VPCS — по принципу PCx-user, где вместо user укажите имя вашей учётной записи, вместо x — порядковый номер устройства.

4. Включите захват трафика на соединении между коммутатором и маршрутизатором.(рис. [-@fig:024])

![Включаем захват + окно Wireshark](image/25.png){#fig:024 width=90%}

5. Запустите все устройства проекта. Откройте консоль всех устройств проекта.

6. Настройте IP-адресацию для интерфейса узла PC1:(рис. [-@fig:025])

![Настроем IP-адресацию ](image/26.png){#fig:025 width=90%}

Далее не будет фотографий, так как из-за проблем с версией VyOS или внутрених ошибок у меня просто не загрузился терменал для работы с VyOS, либо показывал, что он не может загузится из-за памяти. И того, -3 часа, которые не попали на запись, но проблему я так и не смогла решить. Простите. 
...
Далее ответьте на вопросы диалога установки, в котором в большинстве пунктов можно соглашаться с предлагаемыми поумолчанию значениями, нажимая Enter . По завершении диалога перезапустите маршрутизатор, введя команду reboot.
...
10. Остановите захват пакетов в Wireshark. Остановите все устройства в проекте. Завершите работу с GNS
